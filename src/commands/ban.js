...
const { createBanEmbed } = require('../utils/embedCreator.util');
...
module.exports = {
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName('reason').setDescription('give reason to ban')
		)
		.addIntegerOption((option) =>
			option
				.setName('delete-messages')
				.setDescription(
					'How many days of messages to delete, max seven'
				)
		)
	...
	async execute(client, interaction, secret) {
		...
		const deleteMessageSeconds =
			interaction.options.getInteger('delete-messages') * 86400;
		...
		await member
			.send(
				`You've been banned from ${interaction.guild.name} with the reason of : ${reason}`
			)
			.catch(async (err) => {
				if (err.code == '50007') {
					sentDM = false;
